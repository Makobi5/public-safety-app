...
// In _IncidentReportFormPageState class
  ...
  void _applyUgandaSpecificMapping() {
    ...
    setState(() {
      ...
      _detectedRegion =
          UgandaRegionsMapper.getRegionForCoordinates(_latitude!, _longitude!);
      ...
      if (_detectedVillage == null || _detectedVillage == 'Unknown Village') {
        _detectedVillage =
            _detectedDistrict == 'Kabale' ? 'Kabale Area' : 'Unknown Village';
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (lat >= 0.25 && lat <= 0.4 && lon >= 32.5 && lon <= 32.7) {
          setState(() {
            ...
            _detectedVillage = addressParts['neighbourhood'] ??
                addressParts['suburb'] ??
                'Kampala';
        ...
        setState(() {
          _detectedRegion = addressParts['state'] ?? 'Unknown Region';
          _detectedDistrict = addressParts['county'] ??
              addressParts['city'] ??
              'Unknown District';
          _detectedVillage = addressParts['village'] ??
              addressParts['suburb'] ??
...
// Helper method to determine if coordinates are likely in Uganda
  ...
  // API endpoints
  final String _fileUploadEndpoint =
      'https://hkggxkyzyjptapnqbdlc.supabase.co/storage/v1/object/public/incident-files';
  final String _incidentsEndpoint =
      'https://hkggxkyzyjptapnqbdlc.supabase.co/rest/v1/incidents';
  final String _authToken =
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhrZ2d4a3l6eWpwdGFwbnFiZGxjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDE1OTgyNTksImV4cCI6MjA1NzE3NDI1OX0.RSq8Fl40y1PRTl_77UbJWwqbdMIY9mWE7YTH4a-1NsQ';
      ...
      if (result != null) {
        ...
        if (oversizedFiles.isNotEmpty) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  'Some files exceed the $_maxFileSizeInMB MB limit: $fileNames'),
  ...
  Future<String> _uploadFile(PlatformFile file) async {
    try {
      final supabase = Supabase.instance.client;
      final filePath =
          'incident-files/${DateTime.now().millisecondsSinceEpoch}_${file.name}';
      ...
      final publicUrl =
          supabase.storage.from('incident-files').getPublicUrl(filePath);
...
// Update your _buildLocationSection method to include police station selection
              ...
              children: [
                ...
                Center(
                  child: ElevatedButton.icon(
                    onPressed:
                        _isCapturingLocation ? null : _getCurrentLocation,
                          ...
                          child: DropdownButton<String>(
                            ...
                            onChanged: (String? newValue) {
                              setState(() {
                                ...
                                _selectedPoliceStationName =
                                    _policeStations.firstWhere((station) =>
                                        station['id'] == newValue)['name'];
                              });
                            },
                            items: _policeStations
                                .map<DropdownMenuItem<String>>((station) {
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color:
                          _isUploading ? Colors.grey : const Color(0xFF003366),
          ...
          if (_uploadedFiles.isNotEmpty) ...[
            ...
            if (_uploadedFiles.length < _maxFiles) ...[
              ...
              TextButton.icon(
                ...
                label: Text(
                    'Add more files (${_uploadedFiles.length}/$_maxFiles)'),
        ...
        if (_currentPosition == null) {
          final shouldContinue = await showDialog<bool>(
                ...
                builder: (context) => AlertDialog(
                  ...
                  content: const Text(
                      'Some features require location. Continue without?'),
                  actions: [
                    TextButton(
  ...
  Future<Map<String, dynamic>> _prepareIncidentData(
  ...
  void _resolveRegionAndDistrict() {
    ...
    if (districtName != null && districtName != 'Unknown District') {
      if (_detectedRegion == null || _detectedRegion == 'Unknown Region') {
        _detectedRegion =
            UgandaRegionsMapper.getRegionForDistrict(districtName);
      }
    ...
    if ((districtName == null || districtName == 'Unknown District') &&
      ...
      final mappedDistrict =
          UgandaRegionsMapper.getDistrictForVillage(villageName);
      if (mappedDistrict != 'Unknown District') {
        _detectedDistrict = mappedDistrict;
        _detectedRegion =
            UgandaRegionsMapper.getRegionForDistrict(mappedDistrict);
      }
    }
  }
  ...
  void _handleSubmissionError(dynamic error) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content:
            Text('Submission failed: ${error.toString().split(':').first}'),
        backgroundColor: Colors.red,
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  TextFormField(
                                    ...
                                    decoration: InputDecoration(
                                      hintText:
                                          'Enter contact details of any witnesses',
                                      border: OutlineInputBorder(
                                        borderRadius: BorderRadius.circular(8),
                                      ),
