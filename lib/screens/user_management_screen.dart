...
class _UserManagementScreenState extends State<UserManagementScreen>
  ...
  void _debugRoles() {
    ...
    for (var user in _users) {
      print(
          'User: ${user['first_name']} ${user['last_name']}, Role: "${user['role']}"');
    }
  }
  ...
  Future<void> _deleteUser(String userId) async {
    ...
    try {
      ...
      final result = await supabase
          .rpc('force_delete_user', params: {'user_id_param': userId});
  ...
  String _getRoleDisplay(String role) {
    switch (role.toLowerCase()) {
      case 'super_admin': // Added this
        return 'Main Admin / OC';
      case 'admin':
          ...
          builder: (context, setState) => Column(
            ...
            children: [
              RadioListTile<String>(
                title: const Text('Super Admin (Main OC)'), // Added this
                value: 'super_admin',
  ...
  Future<void> _updateUserRole(String userId, String newRole) async {
    ...
    try {
      ...
      print('Starting role update for user $userId to $normalizedRole');

      // Step 1: Update the role in user_profiles table (The Source of Truth for the UI)
      final updateResult = await supabase.rpc('force_update_user_role',
          params: {'user_id_param': userId, 'new_role_param': normalizedRole});

      print('Profile role update result: $updateResult');

      // Step 2: Sync with the admins table (The Source of Truth for Permissions)
      // We check for both 'admin' (Station Admin) and 'super_admin' (District OC)
      if (normalizedRole == 'admin' || normalizedRole == 'super_admin') {
        // Use upsert to either create the admin record or update the existing one
        await supabase.from('admins').upsert({
          ...
          'created_at': DateTime.now().toIso8601String()
            ...
            onConflict:
                'user_id'); // This works because of the UNIQUE constraint we added

        print('Synced user to admins table as $normalizedRole');
      } else {
        // If the new role is NOT an admin type (e.g., standard user),
        // we must remove them from the admins table to revoke permissions
        await supabase.from('admins').delete().eq('user_id', userId);

        print('User is no longer an admin. Removed from admins table.');
      }
      ...
      if (mounted) {
        ...
        // Step 3: Refresh the user list to update the counts and UI badges
        await _fetchUsers();
      }
    } catch (e) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Text(
                          _getRoleDisplay(
                              role), // This will now use case-insensitive comparison
                          style: TextStyle(
